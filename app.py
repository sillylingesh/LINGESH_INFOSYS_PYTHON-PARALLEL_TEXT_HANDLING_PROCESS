...
def api_send_email():
    ...
    stats = get_aggregate_stats()
    pos = stats.get("positive_words", 0)
    neg = stats.get("negative_words", 0)
    neu = stats.get("neutral_words", 0)

    url = os.getenv("url")
    if not url:
        return jsonify({"error": "Email service URL not configured."}), 500

    payload = {
        "to": to_email,
        "subject": "Parallel Text Processor Stats 🚀",
        "body": f"Here are the current stats from the Parallel Text Processor:\n\nPositive Words: {pos}\nNegative Words: {neg}\nNeutral Words: {neu}\n"
    }
